...
- **`com.leatcode.t<N>/`** — One `Solution.kt` per LeetCode problem number (~500+ problems). Each file is standalone; solutions are self-contained functions or classes.
- **`com.leatcode.util/`** — Shared data structures used across LeetCode solutions:
...
- **`com.leatcode.assessment/`** — Solutions from timed assessments
...
Create `app/src/main/java/com/leatcode/t<N>/Solution.kt` with `package com.leatcode.t<N>`. Import `com.leatcode.util.*` if the problem involves trees or linked lists.
